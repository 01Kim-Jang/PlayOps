...
| PM · FE | **장준호** ([@Junho73](https://github.com/Junho73)) | 프로젝트 일정·범위·산출물 총괄, QA 인텔리전스 대시보드 웹 화면(불안정 테스트 랭킹·AI 수정 지식·실패 클러스터 맵·변경 영향도 뷰), AI 검토 화면 개선, 비즈니스 모델 확정 |
| BE · Server | **김윤재** ([@YoonJae00](https://github.com/YoonJae00)) | 선행 데이터 공백 보완, 온톨로지·지식그래프(PostgreSQL) 구축 및 적재, 대시보드·모바일용 조회 API, LangGraph 기반 AI 수정 루프 프로토타입, 테스트 서버·CI/CD 운영 |
| Mobile FE | **장동현** | React Native 모바일 앱 MVP — 실행 결과·결함 현황 조회, 푸시 알림, AI 수정 승인 화면 |
| OSS | **박준서** | 오픈소스 도구 구축·연동 — Redmine(실패 클러스터 결함 등록·상태 동기화), SonarQube Community(코드 품질 지표 수집, AI 수정 전후 비교) |
